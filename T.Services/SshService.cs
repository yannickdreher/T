...
public class SshService : ISshService
{
    ...
    {
        if (_knownHostsService.IsHostKeyKnown(_session.Host, _session.Port, e.HostKey))
        {
            e.CanTrust = true;
            return;
        }
        ...
        if (HostKeyVerificationRequired != null)
        {
            ...
            if (trusted)
            {
                _knownHostsService.AddHostKey(_session.Host, _session.Port, e.HostKeyName, e.HostKey);
            }
        }
        else
        {
