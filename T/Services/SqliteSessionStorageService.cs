...
public class SqliteSessionStorageService : ISessionStorageService
{
    ...
    /// <summary>
    /// Globale Storage-Instanz, von überall zugreifbar
    /// </summary>
    public static SqliteSessionStorageService Current { get; } = new();

    private SqliteSessionStorageService()
    {
    ...
    private static SshSession ReadSession(SqliteDataReader reader) => new()
    {
        ...
        Password = EncryptionService.Decrypt(reader.GetString(5)),
        PrivateKeyPath = reader.GetString(6),
        PrivateKeyPassword = EncryptionService.Decrypt(reader.GetString(7)),
    ...
    private static void AddSessionParameters(SqliteCommand cmd, SshSession session)
    {
        ...
        cmd.Parameters.AddWithValue("@Password", EncryptionService.Encrypt(session.Password));
        cmd.Parameters.AddWithValue("@PrivateKeyPath", session.PrivateKeyPath);
        cmd.Parameters.AddWithValue("@PrivateKeyPassword", EncryptionService.Encrypt(session.PrivateKeyPassword));
